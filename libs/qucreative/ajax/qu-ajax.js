...
"use strict";function adjustZoombox(){try{var o=JSON.parse(match[2]);if(quCreative_main.zoombox_options&&(quCreative_main.old_zoombox_options=$.extend([],zoombox_options)),window.zoombox_default_opts_string){var i=$.extend(!0,{},$.parseJSON(window.zoombox_default_opts_string));"darkfull"==o.type&&(quCreative_main.zoombox_options=$.extend(i,window.init_zoombox_darkfull)),"whitefull"==o.type&&(quCreative_main.zoombox_options=$.extend(i,window.init_zoombox_whitefull)),window.init_zoombox_settings=quCreative_main.zoombox_options}else window.init_zoombox_preset=o.type,"darkfull"==window.init_zoombox_preset&&(window.init_zoombox_settings=$.extend({},window.init_zoombox_darkfull)),"whitefull"==window.init_zoombox_preset&&(window.init_zoombox_settings=$.extend({},window.init_zoombox_whitefull));window.api_zoombox_setoptions,quCreative_main.qcre_init_zoombox=!0}catch(o){}}Object.defineProperty(exports,"__esModule",{value:!0}),exports.adjustZoombox=adjustZoombox;
...
exports.quAjax_view_loadFromNewPage = quAjax_view_loadFromNewPage;
var _qucreative = require("../_qucreative.config");
var _adjustZoombox = require("../_adjustZoombox");
function quAjax_view_loadFromNewPage(ajaxOptions) {
  let $ = jQuery;
  const _body = $('body');
  const theResponse = ajaxOptions.response;
  if (theResponse.indexOf("widget_media_video") > -1) {
    ajaxOptions.scripts_tobeloaded.push(window.qucreative_options.site_url + "/wp-includes/js/mediaelement/mediaelement-and-player.min.js");
    ajaxOptions.scripts_tobeloaded.push(window.qucreative_options.site_url + "/wp-includes/js/mediaelement/wp-mediaelement.min.js");
  }
  var regex_bodyclass = /<body.*?class="(.*?)"/g;
  var regex_bodyclass_page2 = /<body.*?class=".*?(page-(?:blogsingle|homepage|gallery-w-thumbs|normal|contact|about|contact|portfolio|portfolio-single))[ |"]/g;
  ...
  quCreative_main.newclass_body = quCreative_main.newclass_body.replace(/menu-type-\d*/g, "");
  ...
  var match = null;
  while (match = _qucreative.regex_ajax_find_scripts.exec(theResponse)) {
    if (match[1]) {
      var sw = false;
      for (var j = 0; j < quCreative_main.scripts_loaded_arr.length; j++) {
        ...
        if (match[1] == "" || quCreative_main.scripts_loaded_arr[j] == match[1] || ajax_site_url + quCreative_main.scripts_loaded_arr[j] == match[1]) {
          sw = true;
        }
      }
      if (sw == false && match[1]) {
  ...
  while (match = _qucreative.regex_ajax_find_links.exec(theResponse)) {
    if (match[2]) {
      var sw = false;
      for (var j = 0; j < quCreative_main.scripts_loaded_arr.length; j++) {
        if (match[1] || match[2] == "" || quCreative_main.scripts_loaded_arr[j] == match[2] || ajax_site_url + quCreative_main.scripts_loaded_arr[j] == match[2] || String(match[0]).indexOf("stylesheet") == -1) {
          sw = true;
        }
      }
      if (sw == false && match[2]) {
  ...
  const regex_mo = /<div.*?class="qucreative-option-feed"(.*?)>([\s|\S]*?)<\/div>/gim;
  while (match = regex_mo.exec(theResponse)) {
    if (match[1]) {
      ...
      if (match[1].indexOf("zoombox-options") > -1) {
        (0, _adjustZoombox.adjustZoombox)();
      }
...
},{"../_adjustZoombox":1,"../_qucreative.config":2}],4:[function(require,module,exports){
"use strict";

var _helpers = require("../js/common/_helpers");
var _adjustZoombox = require("../_adjustZoombox");
var _qucreative = require("../_qucreative.config");
var _quAjaxView = require("./_qu-ajax-view");
/**
 * called in init
 */
function setupAjax(quCreative_main) {
  ...
  var selector_ajax = ".zfolio-portfolio-classic a.zfolio-item--inner:not(.not-ajax), .portfolio-link--toback-a, a.portfolio-link--title,a.ajax-link";
  ...
  quCreative_main._navCon_520.find("select").eq(0).unbind("change", change_nav_con_520);
  quCreative_main._navCon_520.find("select").eq(0).bind("change", change_nav_con_520);
  function handle_popstate(e) {
    ...
    if (e.state && e.state.href) {
      ...
      if (Object.size(e.state.curr_menu_items) > 0) {
        quCreative_main.$theActualNav.find(".current-menu-item").removeClass("current-menu-item");
        for (var i2 = 0; i2 < Object.size(e.state.curr_menu_items); i2++) {
          quCreative_main.$theActualNav.find("li").eq(e.state.curr_menu_items[i2]).addClass("current-menu-item");
        }
      }
      return false;
    }
  ...
  function click_menu_anchor(e, pargs) {
    ...
    if (qucreative_options.enable_ajax == "on" && thehref == quCreative_main.curr_html) {
      return false;
    }
    if ((0, _helpers.is_touch_device)()) {
      if (_t && _t.hasClass("zoombox-delegated")) {} else {
    ...
    if (window.qucreative_options.enable_ajax == "on" && (!_t || _t.hasClass("not-ajax") == false) && window && margs.force_no_ajax != "on" && allow_html5_link_process) {
      if (thehref == "#" || thehref && thehref.indexOf("#") == 0 || thehref.indexOf("#") > -1 && thehref.split("#")[0] == String(window.location.href).split("#")[0]) {
        ...
        if (window.location.href.indexOf("file://") == 0 || (thehref.indexOf("http://") > -1 || thehref.indexOf("https://") > -1) && thehref.indexOf(ajax_site_url) != 0) {} else {
          clearInterval(quCreative_main.inter_bg_slideshow);
          _body.removeClass("loaded");
          if ((0, _helpers.can_history_api)()) {
            ...
            $.ajax({
              ...
              (0, _quAjaxView.quAjax_view_loadFromNewPage)();
              if ($(".footer-extra-zoombox-items").length) {
                $(".footer-extra-zoombox-items").html(ajaxOptions.new_footer_extra_content_html);
              } else {
                _body.append('<div class="footer-extra-zoombox-items">' + ajaxOptions.new_footer_extra_content_html + '<span style="display: none;">dzs2</span></div>');
              }
              ...
              setTimeout(function () {
                var i = 0;
                nr_scripts_tobeloaded = ajaxOptions.scripts_tobeloaded.length + ajaxOptions.stylesheets_tobeloaded.length;
                function loadFunc(e) {}
                ...
                for (i4 = 0; i4 < ajaxOptions.scripts_tobeloaded.length; i4++) {
                  $.getScript(ajaxOptions.scripts_tobeloaded[i4]).done(function (data, textStatus, jqxhr) {
                ...
                for (i4 = 0; i4 < ajaxOptions.stylesheets_tobeloaded.length; i4++) {
                  ...
                  quCreative_main.scripts_loaded_arr.push(ajaxOptions.stylesheets_tobeloaded[i4]);
                  nr_scripts_tobeloaded--;
                  if (nr_scripts_tobeloaded <= 0) {
                    load_new_page();
                  }
              ...
              if (quCreative_main.bg_transition == "fade") {
                ...
                if (quCreative_main._mainBg.get(0) && quCreative_main._mainBg.get(0).api_destroy) {
              ...
              if (_t.get(0) != window) {
                if (history_first_pushed_state == false) {
                  if (window.location.href.indexOf("file://") === -1) {
                    ...
                    var stateObj = {
                      href: quCreative_main.curr_html
                    };
            ...
            if (_t.get(0) != window) {
              ...
              quCreative_main.$theActualNav.find(".current-menu-item").each(function () {
                var _t = $(this);
                state_curr_menu_items_links.push(quCreative_main.$theActualNav.find("li").index(_t));
              });
              if ((_t && _t.hasClass("donotchange-ajax-menu")) == false) {
                if (_t) {}
                quCreative_main.$theActualNav.find(".current-menu-item,.current-menu-ancestor").removeClass("current-menu-item current-menu-ancestor");
                if (_t.attr("rel") == "home") {
                  if (quCreative_main.$theActualNav.find("a[rel=home]").length > 0) {
                    quCreative_main.$theActualNav.find("a[rel=home]").eq(0).parent().addClass("current-menu-item");
                  }
                }
              ...
              if ((_t && _t.hasClass("donotchange-ajax-menu")) == false) {
                quCreative_main.$theActualNav.find("li > a").each(function () {
                  ...
                  if (_t3.attr("href") == thehref || _t3.attr("href") == ajax_site_url + thehref) {
                    quCreative_main.$theActualNav.find("li").removeClass("current-menu-item");
...
},{"../_adjustZoombox":1,"../_qucreative.config":2,"../js/common/_helpers":5,"./_qu-ajax-view":3}],5:[function(require,module,exports){
...
//# sourceMappingURL=qu-ajax.js.map